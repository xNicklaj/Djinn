...
namespace Mirage.Impostors.Core
{
    ...
    public class HDRPEditorBakingEngine : IBakingEngine
    {
        ...
        private Volume bakingHDRPVolume;
        private object indirectLightingController;

        private void SetupHDRPComponents()
        {
            try
            {
                var hdrpAssembly = Assembly.Load("Unity.RenderPipelines.HighDefinition.Runtime");
                if (hdrpAssembly == null)
                {
                    Debug.LogWarning("[Mirage] HDRP assembly could not be loaded. Ensure HDRP is installed.");
                    return;
                }

                var hdrpVolumeType = typeof(Volume) ;
                if (hdrpVolumeType == null)
                {
                    Debug.LogWarning("[Mirage] HDRP Volume type could not be found. Ensure HDRP is properly installed.");
                    return;
                }

                bakingHDRPVolume = camera.gameObject.AddComponent<Volume>();
                var profileField = hdrpVolumeType.GetProperty("profile");
                var isGlobalField = hdrpVolumeType.GetProperty("isGlobal");

                if (profileField == null || isGlobalField == null)
                {
                    Debug.LogWarning("[Mirage] HDRP Volume properties 'profile' or 'isGlobal' could not be found.");
                    return;
                }

                profileField.SetValue(bakingHDRPVolume, Resources.Load("MirageHDRPBakingSettingsVolumeProfile"));
                isGlobalField.SetValue(bakingHDRPVolume, true);

                var profile = profileField.GetValue(bakingHDRPVolume);
                if (profile == null)
                {
                    Debug.LogWarning("[Mirage] HDRP Volume profile could not be loaded.");
                    return;
                }

                var indirectLightingControllerType = hdrpAssembly.GetType("UnityEngine.Rendering.HighDefinition.IndirectLightingController");
                if (indirectLightingControllerType == null)
                {
                    Debug.LogWarning("[Mirage] HDRP IndirectLightingController type could not be found.");
                    return;
                }
                
                var tryGetGenericMethod = profile.GetType().GetMethods().FirstOrDefault(m => m.Name == "TryGet" && m.IsGenericMethodDefinition && m.GetParameters().Length == 1);
                if (tryGetGenericMethod == null)
                {
                    Debug.LogWarning($"[Mirage] HDRP profile generic method 'TryGet<T>' could not be found on type {profile.GetType().FullName}.");
                    return;
                }

                // Make the generic method with IndirectLightingController
                var tryGetMethod = tryGetGenericMethod.MakeGenericMethod(indirectLightingControllerType);
                var parameters = new object[] { null };
                bool result = (bool)tryGetMethod.Invoke(profile, parameters);
                if (!result)
                {
#if UNITY_2022_1_OR_NEWER
                    Debug.LogWarning("[Mirage] HDRP IndirectLightingController could not be retrieved.");
#endif
                    return;
                }


                indirectLightingController = parameters[0];
                Debug.Log("[Mirage] HDRP components setup successfully.");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[Mirage] An error occurred while setting up HDRP components: {ex.Message}\n{ex.StackTrace}");
            }
        }

        private void SetHDRPIndirectLightingMultiplier(float value)
        {
            if (indirectLightingController != null)
            {
                try
                {
                    var reflectionLightingMultiplierField = indirectLightingController.GetType().GetField("reflectionLightingMultiplier", BindingFlags.Instance | BindingFlags.Public);
                    var floatParameter = reflectionLightingMultiplierField.GetValue(indirectLightingController);
                    var valueProperty = floatParameter.GetType().GetProperty("value", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                    valueProperty.SetValue(floatParameter, value);
                }
                catch (Exception ex)
                {
                    Debug.LogError($"[Mirage] An error occurred while setting the reflectionLightingMultiplier value: {ex.Message}\n{ex.StackTrace}");
                }
            }
        }
        ...
        public HDRPEditorBakingEngine(List<Vector3> povs, List<Mesh> meshes, List<Material[]> materials, List<Matrix4x4> meshesTransforms, int textureSize, float orthographicSize, LightingMethod lightingMethod = LightingMethod.SurfaceEstimation)
        {
        ...
        public override Texture2D ComputeColorMaps()
        {
            ...
            SetHDRPIndirectLightingMultiplier(1f);
        ...
        public override Texture2D ComputeMaskMaps()
        {
            ...
            for (int pov = 0; pov < CameraPositions.Count; ++pov)
            {
                ...
                SetHDRPIndirectLightingMultiplier(0f);
