...
namespace FIMSpace.FOptimizing
{
    ...
    public partial class Optimizer_BaseEditor : Editor
    {
            ...
            else
            {
                ...
                if (!Application.isPlaying)
                {
                    ...
                    if (visibleToOptList)
                    {
                        ...
                        DrawToOptimizeHeader(headerHeight);
        ...
        protected void DrawToOptimizeHeader(int headerHeight)
        {
            string title = Lang("To Optimize List") + " (" + Get.GetToOptimizeCount() + ")";
            var icon = FGUI_Resources.Tex_Sliders;
            int iconsSize = headerHeight - 1;

            EditorGUILayout.BeginHorizontal(FGUI_Resources.HeaderBoxStyle);
            string f = FGUI_Resources.GetFoldSimbol(Get._editor_DrawOptimizeList);

            GUILayout.Label(new GUIContent(" "), GUILayout.Width(1));
            if (icon != null) if (GUILayout.Button(new GUIContent(icon), EditorStyles.label, new GUILayoutOption[2] { GUILayout.Width(iconsSize), GUILayout.Height(iconsSize) })) { Get._editor_DrawOptimizeList = !Get._editor_DrawOptimizeList; }

            if (GUILayout.Button(f + "     " + title + "     " + f, FGUI_Resources.HeaderStyle, GUILayout.Height(headerHeight)))
            {
                if (Event.current != null && (Event.current.button == 1 || Event.current.control))
                {
                    GenericMenu menu = new GenericMenu();
                    menu.AddItem(new GUIContent("Clear Whole 'To Optimize' List"), false, () => { Get.ClearToOptimizeList(); EditorUtility.SetDirty(Get); });
                    menu.ShowAsContext();
                }
                else
                {
                    Get._editor_DrawOptimizeList = !Get._editor_DrawOptimizeList;
                }
            }

            if (icon != null) if (GUILayout.Button(new GUIContent(FGUI_Resources.Tex_Sliders), EditorStyles.label, new GUILayoutOption[2] { GUILayout.Width(iconsSize), GUILayout.Height(iconsSize) })) { Get._editor_DrawOptimizeList = !Get._editor_DrawOptimizeList; }
            GUILayout.Label(new GUIContent(" "), GUILayout.Width(1));
