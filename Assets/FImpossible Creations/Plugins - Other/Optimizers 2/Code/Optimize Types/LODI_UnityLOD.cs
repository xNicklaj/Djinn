        ...
        public void CustomEditorWindow(SerializedProperty prop, LODsControllerBase contr)
        {
            ...
            if (prop != null)
            {
                ...
                if ( GUI.Button(rect, GUIContent.none, EditorStyles.label))
                {
                    ...
                    menu.AddItem(new GUIContent("Assign this Unity LOD Level value to all instances in this optimzier LOD level"), false, () =>
                    {
                        for (int i = 0; i < contr.Optimizer.GetToOptimizeCount(); i++)
                        {
                            var lodInst = contr.Optimizer.GetLODInstance(i, contr.EditorSelectedLODLevel);
                            if ( lodInst is LODI_UnityLOD)
                            {
                                var otherulod = lodInst as LODI_UnityLOD;
                                otherulod.UnityLODLevel = UnityLODLevel;
                            }
                        }

                        EditorUtility.SetDirty(contr.Optimizer);
                    });
