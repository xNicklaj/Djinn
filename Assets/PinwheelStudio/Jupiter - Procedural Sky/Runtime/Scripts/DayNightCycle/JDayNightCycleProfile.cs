    ...
    public class JDayNightCycleProfile : ScriptableObject
    {
        ...
        private static void InitPropertyRemap()
        {
            ...
            foreach (PropertyInfo p in props)
            {
                ...
                ScriptPropertyRemap.Add(name, p);
            }
        }
        ...
        public void Animate(JSky sky, float t)
        {
            ...
            for (int i = 0; i < AnimatedProperties.Count; ++i)
            {
                ...
                if (aProp.Target == JAnimateTarget.Material)
                {
                    int id = 0;
                ...
                else if (aProp.Target == JAnimateTarget.SkyProfile)
                {
                    PropertyInfo scriptProp = null;
                    if (!ScriptPropertyRemap.TryGetValue(aProp.Name, out scriptProp))
                    {
                        continue;
                    }
                    ...
                    if (aProp.CurveOrGradient == JCurveOrGradient.Curve)
                    {
                        scriptProp.SetValue(sky.Profile, aProp.EvaluateFloat(t));
                    }
                    else
                    {
                        scriptProp.SetValue(sky.Profile, aProp.EvaluateColor(t));
                    }
                }
            }
        }
