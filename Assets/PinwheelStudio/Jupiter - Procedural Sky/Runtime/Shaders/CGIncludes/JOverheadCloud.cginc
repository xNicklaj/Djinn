	...
	out fixed4 color)
{
	...
	float3 intersectionPoint = rayDir * rayLength - float3(flowX, 0, flowZ) * flowSpeed * _JupiterTime;
	...
	fixed noiseAmp = 1;
	fixed sign = -1;
	fixed2 span = fixed2(flowX, flowZ) * animationSpeed * _JupiterTime * 0.0001;
