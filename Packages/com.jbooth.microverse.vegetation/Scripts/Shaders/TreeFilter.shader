Shader "Hidden/MicroVerse/VegetationFilter"
{
    ...
    SubShader
    {
        Pass
        {
            ...
            float _DiscStrength;
            float _MinHeight;
            float3 _TextureLayerWeights[TEXCOUNT];
            int _NumTreeIndexes;
            float _TotalWeights;
            int _YCount;
            float _HeightOffset;
            ...
            FragmentOutput frag(v2f i)
            {
                ...
                float2 noiseUV = uv * _NoiseUV.z + _NoiseUV.xy;
                ...
                if (uv.x < 0 || uv.x > 1 || uv.y < 0 || uv.y > 1 || cellIdx > _InstanceCount)
                {
                    o.posWeight = -1;
                    o.randoms = -1;
                    return o;
                ...
                if (w > 0)
                {
                    ...
                    // apply sink
                    height = UnpackHeightmap(_Heightmap.Sample(shared_linear_clamp, uv));
