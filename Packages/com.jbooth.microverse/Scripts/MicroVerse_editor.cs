...
namespace JBooth.MicroVerseCore
{
    public partial class MicroVerse : MonoBehaviour
    {
        ...
        static public Texture2D GetMegasplatTexture(Terrain terrain)
        {
#if __MICROSPLAT__ && __MICROSPLAT_MEGA__
            if (terrain == null) return null;
            var mst = terrain.GetComponent<MicroSplat.MicroSplatTerrain>();
            if (mst == null) return null;
            if (mst.keywordSO != null && mst.keywordSO.IsKeywordEnabled("_MEGASPLATTEXTURE"))
            {
                if (mst.megaSplatMap != null)
                {
                    return mst.megaSplatMap;
                }
            }
            ...
            if (!Application.isPlaying)
            {
                if (options.settings.useSceneCulling)
                {
                    ...
                    foreach (var t in terrains)
                    {
                        ...
                        if(options.settings.controllFoliageDraw)
