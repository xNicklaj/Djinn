...
namespace JBooth.MicroVerseCore
{
    ...
    public class GUIUtil
    {
        ...
        static int DrawSelectionElement(Rect r, int i, int index, Texture2D image, string label, bool labelVisible = true)
        {
            SetupSelectionGrid();
        ...
        public static int SelectionGrid(int index, ref Vector2 scroll, GUIContent[] contents, int imageSize = 64, int maxX = 4, bool supportUnselected = false, bool labelVisible = true)
        {
            EditorGUILayout.BeginHorizontal();
            for (int i = 0; i < contents.Length; ++i)
            {
                Rect r = EditorGUILayout.GetControlRect(GUILayout.Width(imageSize), GUILayout.Height(imageSize));
                index = DrawSelectionElement(r, i, index, contents[i].image as Texture2D, contents[i].text, labelVisible);
                if (i % maxX == maxX-1)
                {
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
        ...
        public class PopupTextureLayerSelector : PopupWindowContent
        {
            ...
            public override void OnGUI(Rect rect)
            {
                ...
                {
                    int nselect = SelectionGrid(selection, ref scroll, contents, 64, 4, true, false);
