    ...
    SubShader
    {
        Pass
        {
            ...
            float BlurSample(float2 stampUV, float height)
            {
                ...
                for (int x = -blurSize; x <= blurSize; x += 1)
                {
                    ...
                    for (int y = -blurSize; y <= blurSize; y += 1)
                    {
                        ...
                        hs = (lerp(ybounds.x + boundsOffset, ybounds.y + boundsOffset, hs) + _YBounds.w) / _RealSize.y;
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float2 uvOffset = (float2(0.5, 0.5) / _StampBounds.xz);
                float depthSample = BlurSample(stampUV - (uvOffset * 0.5), height);
                #if _SUBTRACT
