...
namespace JBooth.MicroVerseCore
{
    ...
    public class ObjectStamp : Stamp, IObjectModifier, ITextureModifier
    {
            ...
            else
            {
                if (parentObjects.Count == 0)
                {
                    ...
                    if (trans == null)
                    {
                        ...
                        trans.parent = MicroVerse.instance.transform;
                        parentObject = trans;
                    }
        ...
        public void RevealHiddenObjects()
        {
            ...
            {
                // Ensure the entry and its transform are valid and the entry is not already processed
                if (entry == null || entry.transform == null || clearedList.Contains(entry.transform) || entry.transform.childCount == 0)
                {
                    continue;
                }
                ...
                for(int i = 0; i < entry.transform.childCount; i++)
                {
                    ...
                    if (child != null && child.gameObject.hideFlags.HasFlag(HideFlags.HideInHierarchy))
                    {
